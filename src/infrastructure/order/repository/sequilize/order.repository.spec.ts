...
describe("Order repository test", () => {
  ...
  it("should update an order", async () => {
    ...
    const orderUpdated = new Order("123", "123", [orderItem2]);
  ...
  it("should find all orders", async () => {
    ...
    const order2 = new Order("456", "123", [order2Item]);
    ...
    expect(foundOrders[0].id).toBe(order.id);
    expect(foundOrders[0].customerId).toBe(order.customerId);
    ...
    expect(foundOrders[1].customerId).toBe(order2.customerId);
    expect(foundOrders[1].items.length).toBe(1);
    expect(foundOrders[1].total()).toBe(order2.total());
  });
});
